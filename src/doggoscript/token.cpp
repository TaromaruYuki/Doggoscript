...
Token::Token(TokenType type, std::optional<std::string> value, std::optional<Position> start_pos, std::optional<Position> end_pos): type(type) {
    if(value.has_value())
    ...
    if(end_pos.has_value()) {
        this->end_pos = Position(end_pos.value());
    }
}

bool Token::is_keyword(const std::string& keyword) {
    return this->type == TokenType::Keyword && std::find(keywords.begin(), keywords.end(), this->value) != keywords.end() && this->value == keyword;
}
