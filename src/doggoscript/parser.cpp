...
ParseResult Parser::parse() {
    ...
    if(!result.should_return() && this->current_token.has_value() && this->current_token.value().type != TokenType::FileEnd) {
...
ParseResult Parser::expr() {
    ...
    BaseNode* node = result.reg(this->bin_op([this] { return this->comp_expr(); }, { TokenType::LogicalAnd, TokenType::LogicalOr }));
...
ParseResult Parser::comp_expr() {
    ...
    BaseNode* node = result.reg(this->bin_op([this] { return this->arith_expr(); }, { TokenType::EqualsTo, TokenType::NotEquals, TokenType::LessThan, TokenType::GreaterThan, TokenType::LessThanEquals, TokenType::GreaterThanEquals }));
...
ParseResult Parser::for_expr() {
    ...
    return *result.success(new ForNode(variable_token, start_value, end_value, step_value, body, static_cast<StatementsNode *>(body)->statements.size() <= 1));
}
...
ParseResult Parser::func_def() {
    ...
    return *result.success(new FunctionDefinitionNode(name_token, arg_name_tokens, body, static_cast<StatementsNode *>(body)->statements.size() <= 1));
}

ParseResult Parser::bin_op(const std::function<ParserFunction>& func, std::vector<TokenType> ops, std::optional<std::function<ParserFunction>> func2) {
    ParseResult result;
    if(!func2.has_value())
        func2 = func;
