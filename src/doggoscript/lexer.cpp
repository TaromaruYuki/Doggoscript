...
LexerResult Lexer::tokenize()  {
    ...
    while(this->current_character != '\0') {
        switch(this->current_character) {
            ...
            case '\n':
            case ';':
                tokens.emplace_back(TokenType::NewLine, std::nullopt, Position(this->position), std::nullopt);
                this->advance();
                break;
            case '+':
                tokens.emplace_back(TokenType::Plus, std::nullopt, Position(this->position), std::nullopt);
                this->advance();
                break;
            case '-':
            ...
            case '(':
                tokens.emplace_back(TokenType::LParen, std::nullopt, Position(this->position), std::nullopt);
                this->advance();
                break;
            case ')':
                tokens.emplace_back(TokenType::RParen, std::nullopt, Position(this->position), std::nullopt);
                this->advance();
                break;
            case '{':
                tokens.emplace_back(TokenType::LCurly, std::nullopt, Position(this->position), std::nullopt);
                this->advance();
                break;
            case '}':
                tokens.emplace_back(TokenType::RCurly, std::nullopt, Position(this->position), std::nullopt);
                this->advance();
                break;
            case '[':
                tokens.emplace_back(TokenType::LSquare, std::nullopt, Position(this->position), std::nullopt);
                this->advance();
                break;
            case ']':
                tokens.emplace_back(TokenType::RSquare, std::nullopt, Position(this->position), std::nullopt);
                this->advance();
                break;
            case '$': {
                this->advance();
            ...
            case '!':
                ...
                break;
            case '>':
                tokens.push_back(this->create_greater_than());
                break;
            case '<':
                tokens.push_back(this->create_less_than());
                break;
            case '.': {
            ...
            default: {
                if(std::isalpha(this->current_character) || this->current_character == '_') {
                    ...
                    this->advance();
                    return {.error = IllegalCharacterError(pos_start, this->position, character)};
                }
            } break;
        }
    }

    tokens.emplace_back(TokenType::FileEnd, std::nullopt, this->position, std::nullopt);
...
std::tuple<Token, std::optional<Token>> Lexer::create_number() {
    ...
    while(this->current_character != '\0' && (isdigit(this->current_character) || this->current_character == '.')) {
        if(this->current_character == '.') {
            if(dot_count == 1) {
                if(prev_character == '.') {
                    through_token = Token(TokenType::Through, std::nullopt, start_pos, Position(this->position));
                    this->advance();
                    num.pop_back();
                    dot_count = 0;
                    break;
...
Token Lexer::create_power() {
    ...
    if(this->current_character == '*') {
        this->advance();
        return {TokenType::Power, std::nullopt, start_pos, Position(this->position)};
    }
...
Token Lexer::create_string() {
    ...
    while(this->current_character != '\0' && (this->current_character != '"' || escape_character)) {
        if(escape_character) {
            if(escape_characters.find(this->current_character) != escape_characters.end()) {
...
Token Lexer::create_identifier() {
    ...
    while(this->current_character != '\0' && (std::isalnum(this->current_character) || this->current_character == '_')) {
        identifier.push_back(this->current_character);
        this->advance();
    }

    Token token(TokenType::Identifier, identifier, start_pos, Position(this->position));
...
Token Lexer::create_not_equals() {
    ...
    if(this->current_character == '=') {
        this->advance();
        return {TokenType::NotEquals, std::nullopt, start_pos, Position(this->position)};
    }

    return {TokenType::LogicalNot, std::nullopt, start_pos, Position(this->position)};
}
...
Token Lexer::create_greater_than() {
    ...
    if(this->current_character == '=') {
        this->advance();
        return {TokenType::GreaterThanEquals, std::nullopt, start_pos, Position(this->position)};
    }

    return {TokenType::GreaterThan, std::nullopt, start_pos, Position(this->position)};
}
...
Token Lexer::create_less_than() {
    ...
    if(this->current_character == '=') {
        this->advance();
        return {TokenType::LessThanEquals, std::nullopt, start_pos, Position(this->position)};
    }

    return {TokenType::LessThan, std::nullopt, start_pos, Position(this->position)};
}
...
std::tuple<std::optional<Token>, std::optional<BaseError>> Lexer::create_and() {
    ...
    if(this->current_character == '&') {
        this->advance();
        return {Token(TokenType::LogicalAnd, std::nullopt, start_pos, Position(this->position)), std::nullopt};
    }
...
std::tuple<std::optional<Token>, std::optional<BaseError>> Lexer::create_or() {
    ...
    if(this->current_character == '|') {
        this->advance();
        return {Token(TokenType::LogicalOr, std::nullopt, start_pos, Position(this->position)), std::nullopt};
    }

    return {std::nullopt, ExpectedCharacterError(start_pos, this->position, '|')};
}

std::tuple<std::optional<Token>, std::optional<BaseError>> Lexer::create_through() {
    Position start_pos(this->position);
    this->advance();

    if(this->current_character == '.') {
        this->advance();
        return {Token(TokenType::Through, std::nullopt, start_pos, Position(this->position)), std::nullopt};
    }
