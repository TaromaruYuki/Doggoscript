...
StringClass::StringClass(std::string initial_value) : BuiltInClass("String") {
    ...
    this->symbol_table->set(
        "length",
        new BuiltInFunction(
            "length", {}, [this](std::vector<Object*> args) -> RuntimeResult {
                return *RuntimeResult().success(
                    NumberClass::new_instance(this->value.length()));
            }));

    this->symbol_table->set(
        "lower",
        new BuiltInFunction(
            "lower", {}, [this](std::vector<Object*> args) -> RuntimeResult {
    ...
    this->symbol_table->set(
        "upper",
        new BuiltInFunction(
            "upper", {}, [this](std::vector<Object*> args) -> RuntimeResult {
    ...
    this->symbol_table->set(
        ...
        new BuiltInFunction(
            "replace", { "old", "new" },
            [this](std::vector<Object*> args) -> RuntimeResult {
                RuntimeResult result;

                BaseClass* old_cls = static_cast<Instance*>(args[0])->cls;
                BaseClass* new_cls = static_cast<Instance*>(args[1])->cls;

                if(old_cls->cls_type != BuiltInType::String ||
                   new_cls->cls_type != BuiltInType::String)
                ...
                size_t start_pos = 0;
                while((start_pos = replaced.find(old, start_pos)) !=
                      std::string::npos) {
            ...
            [this](std::vector<Object*> args) -> RuntimeResult {
                ...
                if(index_cls->cls_type != BuiltInType::Number)
                    return *result.failure(
                        ArgumentError(*this->start_pos, *this->end_pos,
                ...
                return *result.success(
                    this->new_instance(std::string(1, this->value[index])));
            }));
}
