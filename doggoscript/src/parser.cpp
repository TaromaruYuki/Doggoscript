...
ParseResult Parser::statements() {
    ...
    Position               start_pos =
        Position(this->current_token.value().start_pos.value());

    while(this->current_token.has_value() &&
          this->current_token.value().type == TokenType::NewLine) {
    ...
    return *result.success(new StatementsNode(
        statements, start_pos, this->current_token.value().end_pos.value()));
}
...
ParseResult Parser::statement() {
    ...
    if(current_token.is_keyword("return")) {
        ...
        return *result.success(
            new ReturnNode(return_node, start_pos,
                           this->current_token.value().end_pos.value()));
    } else if(current_token.is_keyword("continue")) {
        result.register_advancement();
        this->advance();

        return *result.success(new ContinueNode(
            start_pos, this->current_token.value().end_pos.value()));
    } else if(current_token.is_keyword("break")) {
        result.register_advancement();
        this->advance();

        return *result.success(new BreakNode(
            start_pos, this->current_token.value().end_pos.value()));
        ...
        return *result.success(new IncludeNode(
            path, start_pos, this->current_token.value().end_pos.value()));
    }
...
ParseResult Parser::expr() {
    ...
    if(current_token.is_keyword("val")) {
        ...
        while(this->current_token.value().type == TokenType::Arrow) {
            ...
            if(this->current_token.value().type != TokenType::Variable) {
                return *result.failure(InvalidSyntaxError(
                    current_token.start_pos.value(),
                    current_token.end_pos.value(), "Expected variable name"));
            }
        ...
        return *result.success(
            new VariableAssignmentNode(variable_token, children, value));
    }

    BaseNode* node = result.reg(
        this->bin_op([this] { return this->comp_expr(); },
...
ParseResult Parser::comp_expr() {
    ...
    BaseNode* node = result.reg(this->bin_op(
        [this] { return this->arith_expr(); },
...
ParseResult Parser::atom() {
    ...
    if(value.type == TokenType::Int || value.type == TokenType::Float) {
        ...
        while(this->current_token.has_value() &&
              this->current_token.value().type == TokenType::Arrow) {
            result.register_advancement();
            this->advance();

            if(this->current_token.value().type != TokenType::Variable &&
               this->current_token.value().type != TokenType::Identifier) {
                return *result.failure(InvalidSyntaxError(
        ...
        if(this->current_token.has_value() &&
           this->current_token.value().is_keyword("is")) {
            result.register_advancement();
            this->advance();
            ...
            return *result.success(
                new VariableReassignmentNode(value, children, expr));
        }
...
ParseResult Parser::list_expr() {
    ...
    return *result.success(new ListNode(
        elements, start_pos, this->current_token.value().end_pos.value()));
}
...
ParseResult Parser::if_expr() {
    ...
    if(!this->current_token.value().is_keyword("if")) {
        return *result.failure(InvalidSyntaxError(
            this->current_token.value().start_pos.value(),
            this->current_token.value().end_pos.value(), "Expected 'if'"));
    }
    ...
    cases.emplace_back(condition, expr,
                       static_cast<StatementsNode*>(expr)->statements.size() <=
                           1);
    ...
    while(this->current_token.value().is_keyword("else")) {
        ...
        if(this->current_token.value().is_keyword("if")) {
            ...
            cases.emplace_back(
                condition, expr,
                static_cast<StatementsNode*>(expr)->statements.size() <= 1);
    ...
    if(else_case != nullptr) {
        else_case_should_return =
            static_cast<StatementsNode*>(else_case)->statements.size() <= 1;
...
ParseResult Parser::for_expr() {
    ...
    if(!this->current_token.value().is_keyword("for")) {
        return *result.failure(InvalidSyntaxError(
            this->current_token.value().start_pos.value(),
            this->current_token.value().end_pos.value(), "Expected 'for'"));
    }
    ...
    return *result.success(new ForNode(
        variable_token, start_value, end_value, step_value, body,
...
ParseResult Parser::while_expr() {
    ...
    if(!this->current_token.value().is_keyword("while")) {
        return *result.failure(InvalidSyntaxError(
            this->current_token.value().start_pos.value(),
            this->current_token.value().end_pos.value(), "Expected 'while'"));
    }
    ...
    return *result.success(new WhileNode(
        condition, body,
        static_cast<StatementsNode*>(body)->statements.size() <= 1));
}
...
ParseResult Parser::func_def() {
    ...
    if(!this->current_token.value().is_keyword("fn")) {
        return *result.failure(InvalidSyntaxError(
            this->current_token.value().start_pos.value(),
            this->current_token.value().end_pos.value(), "Expected 'fn'"));
    }
    ...
    return *result.success(new FunctionDefinitionNode(
        name_token, arg_name_tokens, body,
        static_cast<StatementsNode*>(body)->statements.size() <= 1, true));
}

ParseResult Parser::bin_op(const std::function<ParserFunction>&         func,
                           std::vector<TokenType>                       ops,
                           std::optional<std::function<ParserFunction>> func2) {
    ParseResult result;
    if(!func2.has_value()) func2 = func;
    BaseNode* left = result.reg(func());
    if(result.should_return()) return result;

    while(this->current_token.has_value() &&
          std::find(ops.begin(), ops.end(), this->current_token.value().type) !=
              ops.end()) {
...
ParseResult Parser::dict_expr() {
    ...
    return *result.success(new DictNode(
        elements, start_pos, this->current_token.value().end_pos.value()));
}
...
ParseResult Parser::class_def() {
    ...
    if(!this->current_token.value().is_keyword("class")) {
        return *result.failure(InvalidSyntaxError(
            this->current_token.value().start_pos.value(),
            this->current_token.value().end_pos.value(), "Expected 'class'"));
    }
    ...
    for(BaseNode* statement : static_cast<StatementsNode*>(body)->statements) {
        if(statement->type == NodeType::FunctionDefinitionNode) {
            static_cast<FunctionDefinitionNode*>(statement)
