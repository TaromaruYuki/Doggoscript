...
class BaseError {
  ...
  public:
    ...
    BaseError(Position start_pos, Position end_pos, std::string error_name,
              std::string details) :
        start_pos(start_pos),
        end_pos(end_pos), error_name(error_name), details(details) {}
    ...
    std::string string_with_arrows() {
        ...
        for(int i = 0; i < line_count; i++) {
            ...
            if(i != line_count - 1) col_end = line.length() - 1;
...
class IllegalCharacterError : public BaseError {
  public:
    IllegalCharacterError(Position start_pos, Position end_pos,
                          char character) :
        BaseError(start_pos, end_pos, "Illegal Character",
                  "'" + std::to_string(character) + "'") {}
};

class ExpectedCharacterError : public BaseError {
  public:
    ExpectedCharacterError(Position start_pos, Position end_pos,
                           char character) :
        BaseError(start_pos, end_pos, "Expected Character",
                  "'" + std::to_string(character) + "'") {}
};

class InvalidSyntaxError : public BaseError {
  public:
    InvalidSyntaxError(Position start_pos, Position end_pos,
                       std::string details) :
...
class IllegalOperationError : public BaseError {
  public:
    IllegalOperationError(Position start_pos, Position end_pos,
                          std::string details) :
...
class FileNotFoundError : public BaseError {
  public:
    FileNotFoundError(Position start_pos, Position end_pos,
                      std::string details) :
