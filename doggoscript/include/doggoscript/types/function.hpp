...
enum class FunctionType { UserDefined, BuiltIn };

struct BaseFunction : public Object {
    std::string  name;
    FunctionType func_type;

    explicit BaseFunction(std::string name) : name(name), Object() {
        this->type = ObjectType::Function;
    }

    std::string str() { return "<function " + this->name + ">"; }
    ...
    RuntimeResult check_args(std::vector<std::string> arg_names,
                             std::vector<Object*>     args) {
        RuntimeResult result;
        if(args.size() != arg_names.size()) {
            return *result.failure(ArgumentError(
                *this->start_pos, *this->end_pos,
                "Expected " + std::to_string(arg_names.size()) +
    ...
    static void populate_args(std::vector<std::string> arg_names,
                              std::vector<Object*> args, Context& exec_ctx) {
    ...
    RuntimeResult check_and_populate_args(std::vector<std::string> arg_names,
                                          std::vector<Object*>     args,
                                          Context&                 exec_ctx) {
        RuntimeResult result;
...
struct Function : public BaseFunction {
    ...
    Function(std::string name, BaseNode* body_node,
             std::vector<std::string> arg_names, bool should_auto_return) :
        BaseFunction(name),
        body_node(body_node), arg_names(arg_names),
        should_auto_return(should_auto_return) {
        this->func_type = FunctionType::UserDefined;
    }
    ...
    RuntimeResult operator()(std::vector<Object*> args) override {
        ...
        result.reg(
            this->check_and_populate_args(this->arg_names, args, exec_ctx));
        if(result.error.has_value()) return result;

        std::optional<Object*> value =
            result.reg(interpreter.visit(this->body_node, exec_ctx));
...
struct BuiltInFunction : public BaseFunction {
    ...
    BuiltInFunction(std::string name, std::vector<std::string> arg_names,
                    std::function<RuntimeResult(std::vector<Object*>)> func) :
        BaseFunction(name),
        arg_names(arg_names), func(func) {
        this->func_type = FunctionType::BuiltIn;
    }
