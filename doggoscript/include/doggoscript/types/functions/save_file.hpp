...
struct SaveFileFunction : public BuiltInFunction {
    SaveFileFunction() :
        BuiltInFunction("save_file", { "file_name", "content" },
                        SaveFileFunction::call) {}
    ...
    static RuntimeResult call(std::vector<Object*> args) {
        ...
        if(file_name_cls->cls_type != BuiltInType::String)
            return *result.failure(
                ArgumentError(*args[0]->start_pos, *args[0]->end_pos,
        ...
        if(!file) {
            return *result.failure(
                BaseError(*args[0]->start_pos, *args[0]->end_pos, "File Error",
                          "File '" + file_name->value +
                              "' has threw a error. File may be invalid."));
        }
