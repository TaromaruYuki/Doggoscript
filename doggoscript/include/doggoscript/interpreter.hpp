...
struct RuntimeResult {
    ...
    bool should_return() const {
        return this->error.has_value() ||
               ...
               this->loop_should_continue || this->loop_should_break;
    }
};
...
class Interpreter {
  ...
  private:
    ...
    RuntimeResult visit_VariableAssignmentNode(VariableAssignmentNode* node,
                                               Context&                context);

    RuntimeResult visit_VariableAccessNode(VariableAccessNode* node,
                                           Context&            context);

    RuntimeResult visit_VariableReassignmentNode(VariableReassignmentNode* node,
                                                 Context& context);

    RuntimeResult visit_BinaryOperationNode(BinaryOperationNode* node,
                                            Context&             context);

    RuntimeResult visit_UnaryOperationNode(UnaryOperationNode* node,
                                           Context&            context);

    RuntimeResult visit_FunctionDefinitionNode(FunctionDefinitionNode* node,
                                               Context&                context);
    ...
    RuntimeResult visit_ClassDefinitionNode(ClassDefinitionNode* node,
                                            Context&             context);
