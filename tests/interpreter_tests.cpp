...
TEST(Interpreter, Equations) {
    std::map<std::string, double> tests = { { "1 + 2 * 3", 7 },
                                            { "(1 + 2) * 3", 9 },
                                            { "3 ** 5", 243 },
                                            { "2 * (3 - 5) ** 3 / 2 + 1", -7 },
                                            { "-1 * 2", -2 } };
...
TEST(Interpreter, Dictionary) {
    std::map<std::string, std::string> tests = {
        ...
        { "{\"err\" -> {\"code\" -> 404, \"msg\" -> \"Not found\"}, \"content\" -> \"\"}",
          "{\"err\" -> {\"code\" -> 404.000000, \"msg\" -> \"Not found\"}, \"content\" -> \"\"}" }
    };
...
TEST(Interpreter, IfStatement) {
    std::map<std::string, double> tests {
        { "if 5 = 5 { 1 }", 1 },
        { "if 1 = 3 { 1 } else { 10 }", 10 },
        { "if 1 > 5 { 1 } else if 1 < -1 { 2 } else if 1 = 1 { 3 } else { 4 }",
          3 },
        { "if !true { 1 } else { 2 }", 2 },
        { "if 5 <= 5 { 1 }", 1 },
        { "if 8 >= 5 { 1 }", 1 },
...
TEST(Interpreter, ForStatement) {
    ...
    DoggoscriptResult result = run(
        "val $res is 0; for $i in 0..50 { $res is $res + $i }", symbol_table);
    ASSERT_FALSE(result.error.has_value())
        << "Error: " << result.error.value().str();
    ASSERT_TRUE(result.result.has_value());
    ...
    result = run("val $res is 0; for $i in 0..50..2 { $res is $res + $i }",
                 symbol_table);
    ASSERT_FALSE(result.error.has_value())
        << "Error: " << result.error.value().str();
    ASSERT_TRUE(result.result.has_value());
    ...
    result = run("val $res is 0; for $i in 10..0..-1 { $res is $res + $i }",
                 symbol_table);
    ASSERT_FALSE(result.error.has_value())
        << "Error: " << result.error.value().str();
    ASSERT_TRUE(result.result.has_value());
...
TEST(Interpreter, WhileStatement) {
    ...
    DoggoscriptResult result = run(
        "val $res is 0; while $res != 100 { $res is $res + 1 }", symbol_table);
    ASSERT_FALSE(result.error.has_value())
        << "Error: " << result.error.value().str();
    ASSERT_TRUE(result.result.has_value());
    result = run("$res", symbol_table);
    ...
    result = run("val $res is 0; while $res = 100 { $res is $res + 1 }",
                 symbol_table);
    ASSERT_FALSE(result.error.has_value())
        << "Error: " << result.error.value().str();
    ASSERT_TRUE(result.result.has_value());
...
TEST(Interpreter, Classes) {
    ...
    std::cout << "Testing class method 'greeting' on class 'Person'"
              << std::endl;
    ...
    std::cout << "Testing class method 'birthday' on class 'Person'"
              << std::endl;
