...
TEST(Parser, Integer) {
    ...
    ASSERT_FALSE(parse_result.error.has_value()) << "Error: " << parse_result.error.value().str();
...
TEST(Parser, AddTwoNumbers) {
    ...
    ASSERT_EQ(statement_node->statements[0]->type, NodeType::BinaryOperationNode);
    auto *binary_operation_node = static_cast<BinaryOperationNode *>(statement_node->statements[0]);
...
TEST(Parser, ReturnNodeWithValue) {
    ...
    ASSERT_EQ(statements_node->statements[0]->type, NodeType::ReturnNode);
    auto *return_node = static_cast<ReturnNode *>(statements_node->statements[0]);
...
TEST(Parser, VariableAssignment) {
    ...
    auto *statements_node = static_cast<StatementsNode *>(parse_result.node);
    ASSERT_EQ(statements_node->statements.size(), 1);
    ASSERT_EQ(statements_node->statements[0]->type, NodeType::VariableAssignmentNode);

    auto *var_def_node = static_cast<VariableAssignmentNode *>(statements_node->statements[0]);
...
TEST(Parser, VariableReassignmentNode) {
    ...
    auto *statements_node = static_cast<StatementsNode *>(parse_result.node);
    ASSERT_EQ(statements_node->statements.size(), 1);
    ASSERT_EQ(statements_node->statements[0]->type, NodeType::VariableReassignmentNode);

    auto *var_reassign_node = static_cast<VariableReassignmentNode *>(statements_node->statements[0]);
...
TEST(Parser, VariableReassignmentWithChild) {
    ...
    auto *var_def_node = static_cast<VariableReassignmentNode *>(statements_node->statements[0]);
    ASSERT_EQ(var_def_node->token->value, "$a");
    ASSERT_EQ(var_def_node->value->type, NodeType::NumberNode);
    ASSERT_EQ(var_def_node->value->token->value, "123");
    ASSERT_EQ(var_def_node->children.size(), 1);
...
TEST(Parser, VariableAccessNode) {
    ...
    auto *statements_node = static_cast<StatementsNode *>(parse_result.node);
    ASSERT_EQ(statements_node->statements.size(), 1);
    ASSERT_EQ(statements_node->statements[0]->type, NodeType::VariableAccessNode);

    auto *var_access_node = static_cast<VariableAccessNode *>(statements_node->statements[0]);
...
TEST(Parser, LogicalNot) {
    ...
    auto *statements_node = static_cast<StatementsNode *>(parse_result.node);
    ASSERT_EQ(statements_node->statements.size(), 1);
    ASSERT_EQ(statements_node->statements[0]->type, NodeType::UnaryOperationNode);

    auto *unary_op_node = static_cast<UnaryOperationNode *>(statements_node->statements[0]);
...
TEST(Parser, IfStatement) {
    ...
    ASSERT_EQ(std::get<IF_COND>(if_node->cases[0])->type, NodeType::BinaryOperationNode);
    ASSERT_TRUE(std::get<IF_RET>(if_node->cases[0]));

    auto *if_cond_0 = static_cast<StatementsNode *>(std::get<IF_EXPR>(if_node->cases[0]));
...
TEST(Parser, IfElseIfStatement) {
    ...
    ASSERT_EQ(std::get<IF_COND>(if_node->cases[1])->type, NodeType::BinaryOperationNode);
    ASSERT_TRUE(std::get<IF_RET>(if_node->cases[1]));

    auto *if_cond_1 = static_cast<StatementsNode *>(std::get<IF_EXPR>(if_node->cases[1]));
...
TEST(Parser, IfElseStatement) {
    ...
    auto *else_cond = static_cast<StatementsNode *>(std::get<IF_EXPR - 1>(if_node->else_case.value()));
...
TEST(Parser, IfElseIfElseStatement) {
    Lexer lexer("test", "if true = true { 1 } else if true = true { 2 } else { 3 }");
    LexerResult lex_result = lexer.tokenize();
...
TEST(Parser, FunctionDefinitionNoArgs) {
    ...
    ASSERT_EQ(statements_node->statements[0]->type, NodeType::FunctionDefinitionNode);
    auto *function_node = static_cast<FunctionDefinitionNode *>(statements_node->statements[0]);
    ...
    auto *function_body = static_cast<StatementsNode *>(function_node->body_node);
...
TEST(Parser, FunctionDefinitionWithArgs) {
    ...
    auto *function_body = static_cast<StatementsNode *>(function_node->body_node);
    ASSERT_EQ(function_body->statements.size(), 1);
    ASSERT_EQ(function_body->statements[0]->type, NodeType::BinaryOperationNode);
}
...
TEST(Parser, ClassNoConstructor) {
    ...
    ASSERT_EQ(statements_node->statements[0]->type, NodeType::ClassDefinitionNode);
    auto *class_node = static_cast<ClassDefinitionNode *>(statements_node->statements[0]);
...
TEST(Parser, ClassWithConstructorArgs) {
    ...
    auto *constructor_node = static_cast<FunctionDefinitionNode *>(body_node->statements[0]);
