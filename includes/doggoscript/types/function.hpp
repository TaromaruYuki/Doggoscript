...
struct BaseFunction : public Object {
        ...
        if (args.size() != arg_names.size()) {
            return *result.failure(ArgumentError(
                    *this->start_pos, *this->end_pos,
                    "Expected " + std::to_string(arg_names.size()) + " arguments but got " +
    ...
    RuntimeResult
    check_and_populate_args(std::vector<std::string> arg_names, std::vector<Object *> args, Context &exec_ctx) {
        RuntimeResult result;
...
struct Function : public BaseFunction {
    ...
    Function(std::string name, BaseNode *body_node, std::vector<std::string> arg_names, bool should_auto_return)
            : BaseFunction(name), body_node(body_node), arg_names(arg_names),
              should_auto_return(should_auto_return) { this->func_type = FunctionType::UserDefined; }
