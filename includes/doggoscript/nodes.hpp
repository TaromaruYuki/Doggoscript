...
struct IfNode : public BaseNode {
    ...
    IfNode(std::vector<std::tuple<BaseNode *, BaseNode *, bool>> cases,
           std::optional<std::tuple<BaseNode *, bool>> else_case = std::nullopt) : BaseNode() {
...
struct ForNode : public BaseNode {
    ...
    ForNode(Token &var_name, BaseNode *start_value, BaseNode *end_value, BaseNode *step_value, BaseNode *body,
            bool should_return_value) : BaseNode() {
